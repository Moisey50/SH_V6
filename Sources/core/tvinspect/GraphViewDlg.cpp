...
DlgItem GVDI[] =
{
  { IDC_TABVIEW , BOTTOM_ALIGN | RIGHT_ALIGN , { 0 , 0 , 0 , 0 } , ( DWORD ) -1 } ,
  { IDC_RUN , BOTTOM_MOVE , { 0 , 0 , 0 , 0 } , ( DWORD ) -1 } ,
  { IDC_STOP , BOTTOM_MOVE , { 0 , 0 , 0 , 0 } , ( DWORD ) -1 } ,
  { IDC_SAVE_CURRENT , BOTTOM_MOVE , { 0 , 0 , 0 , 0 } , ( DWORD ) -1 } ,
  { IDC_CLOSE_TAB , BOTTOM_MOVE , { 0 , 0 , 0 , 0 } , ( DWORD ) -1 } ,
};
...
BEGIN_MESSAGE_MAP(CGraphViewDlg, CResizableDialog)
  ...
  ON_BN_CLICKED( IDC_SAVE_CURRENT , &CGraphViewDlg::OnSaveCurrentGraph )
...
void CGraphViewDlg::OnDestroy()
{
	...
	int cItems = (int)m_DebugTemplates.GetSize();
	while (cItems--)
	{
...
void CGraphViewDlg::OnSaveCurrentGraph()
{
  IGraphbuilder* pBuilder = GetActiveBuilder() ;
