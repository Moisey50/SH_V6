//  $File : yuv12.cpp - base yuv12 convbersion utility
...
__forceinline bool _compress2yuv12_15(pTVFrame in)
{
...
__forceinline bool _compress2yuv12_8(pTVFrame in)
{
...
__forceinline void _makeRGB(
  LPBYTE pntr, DWORD& R, DWORD& G, DWORD& B , int& rsum , int& gsum , int& bsum )
{
  bsum += B= *(pntr);
  gsum += G= *(pntr+1);
  rsum += R= *(pntr+2);
...
__forceinline void _makeRGB(
  LPBYTE pntr, DWORD& R, DWORD& G, DWORD& B , DWORD& rsum , DWORD& gsum , DWORD& bsum )
{
  bsum += B= *(pntr);
  gsum += G= *(pntr+1);
  rsum += R= *(pntr+2);
...
__forceinline bool _compress2yuv12_24(pTVFrame in)
{
  ASSERT(in->lpData==NULL); //Just that format supported...
  ...
  int oldHeight=oldBMIH->biHeight;
  int newWidth = oldWidth & ~(1) ;
  int newHeight = oldHeight & ~(1) ;
  int newYSize = newWidth * newHeight ;
  int oldWidthB = (((oldWidth * oldBMIH->biBitCount) + 31) / 32 * 4) ;
  int oldWidthB2 = oldWidthB * 2 ;
  int oldWidthB3 = oldWidthB * 3 ;
  int oldImageSize = oldWidthB * newHeight ;
  int newImageSize = (newYSize * 3)/2 ;
  newBMIH=(LPBITMAPINFOHEADER)malloc(oldBMIH->biSize+newImageSize);
  memcpy(newBMIH,oldBMIH,oldBMIH->biSize);
  LPBYTE newData=(LPBYTE)newBMIH+oldBMIH->biSize;
  LPBYTE newDataPntr=newData;
  LPBYTE colUDataOffset = newData + newYSize ;
  LPBYTE colVDataOffset = colUDataOffset + (newYSize/4);
  LPBYTE oldDataPntr=(LPBYTE)oldBMIH+oldBMIH->biSize
    + PaletteSize(oldBMIH) + oldImageSize - oldWidthB ;

  int boxDepth = newHeight/2 ;
  int boxWidth = newWidth/2;
  DWORD R,G,B ;
  int rS, gS, bS;
  for (int y=0; y<boxDepth; y++)
  {
    for (int x=0; x<boxWidth; x++)
    {
      ...
      _makeRGB(oldDataPntr, R , G , B , rS , gS , bS );  
      *newDataPntr=_makeY(R,G,B);
      _makeRGB(oldDataPntr+3, R , G , B , rS , gS , bS ); 
      *(newDataPntr+1)=_makeY(R,G,B);

      _makeRGB(oldDataPntr-oldWidthB, R , G , B , rS , gS , bS ); 
      *(newDataPntr+newWidth)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB+3, R , G , B , rS , gS , bS ); 
      *(newDataPntr+newWidth+1)=_makeY(R,G,B);


      *colVDataOffset=(unsigned char)(((VR_RATIO(rS)+VG_RATIO(gS)+VB_RATIO(bS))>>17)+128);
      *colUDataOffset=(unsigned char)(((UR_RATIO(rS)+UG_RATIO(gS)+UB_RATIO(bS))>>17)+128);
    ...
    newDataPntr += 3 * newWidth ;
    oldDataPntr -= oldWidthB * 2 + newWidth * 3 ;
  }
  free(oldBMIH);
  in->lpBMIH=newBMIH;
  in->lpBMIH->biHeight=newHeight;
  in->lpBMIH->biWidth=newWidth;
  in->lpBMIH->biSizeImage=newImageSize;
  return(true);
}

__forceinline bool _compress2yuv12_32(pTVFrame in)
{
  ASSERT(in->lpData==NULL); //Just that format supported...
  ...
  for (int y=0; y<boxDepth; y++)
  {
    for (int x=0; x<boxWidth; x++)
    {
      ...
      _makeRGB(oldDataPntr, R , G , B , rS , gS , bS );  
      *newDataPntr=_makeY(R,G,B);
      _makeRGB(oldDataPntr+4, R , G , B , rS , gS , bS );  
      *(newDataPntr+1)=_makeY(R,G,B);
      _makeRGB(oldDataPntr+8, R , G , B , rS , gS , bS );  
      *(newDataPntr+2)=_makeY(R,G,B);
      _makeRGB(oldDataPntr+12, R , G , B , rS , gS , bS );  
      *(newDataPntr+3)=_makeY(R,G,B);

      _makeRGB(oldDataPntr-oldWidthB, R , G , B , rS , gS , bS );  
      *(newDataPntr+newWidth)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB+4, R , G , B , rS , gS , bS );  
      *(newDataPntr+newWidth+1)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB+8, R , G , B , rS , gS , bS );  
      *(newDataPntr+newWidth+2)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB+12, R , G , B , rS , gS , bS );  
      *(newDataPntr+newWidth+3)=_makeY(R,G,B);

      _makeRGB(oldDataPntr-oldWidthB2, R , G , B , rS , gS , bS );  
      *(newDataPntr+2*newWidth)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB2+4, R , G , B , rS , gS , bS );  
      *(newDataPntr+2*newWidth+1)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB2+8, R , G , B , rS , gS , bS );  
      *(newDataPntr+2*newWidth+2)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB2+12, R , G , B , rS , gS , bS );  
      *(newDataPntr+2*newWidth+3)=_makeY(R,G,B);

      _makeRGB(oldDataPntr-oldWidthB3, R , G , B , rS , gS , bS );  
      *(newDataPntr+3*newWidth)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB3+4, R , G , B , rS , gS , bS );  
      *(newDataPntr+3*newWidth+1)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB3+8, R , G , B , rS , gS , bS );  
      *(newDataPntr+3*newWidth+2)=_makeY(R,G,B);
      _makeRGB(oldDataPntr-oldWidthB3+12, R , G , B , rS , gS , bS );  
      *(newDataPntr+3*newWidth+3)=_makeY(R,G,B);
      *colVDataOffset=(unsigned char)((VR_RATIO(rS)+VG_RATIO(gS)+VB_RATIO(bS))>>19)+128;
      *colUDataOffset=(unsigned char)((UR_RATIO(rS)+UG_RATIO(gS)+UB_RATIO(bS))>>19)+128;
...
bool compress2yuv12(pTVFrame in)
{
  ...
  bool result=false;
  if (in->lpBMIH->biBitCount==16) result=_compress2yuv12_15(in);
  else if (in->lpBMIH->biBitCount==24) result=_compress2yuv12_24(in);
  else if (in->lpBMIH->biBitCount==8) result=_compress2yuv12_8(in);
  else if (in->lpBMIH->biBitCount==32) result=_compress2yuv12_32(in);

  if (result)
  {
    in->lpBMIH->biCompression=BI_YUV12;
