// usb_relay.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE usb_relay_gadgetDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("usb_relay_gadget.Dll Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(usb_relay_gadgetDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(usb_relay_gadgetDll);
        // Init plugin
        FXString fName=::FxGetModuleName(usb_relay_gadgetDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("usb_relay.DLL Terminating!\n");
		// Terminate the library before destructor will called
		AfxTermExtensionModule(usb_relay_gadgetDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(UsbRelayGadget, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(UsbRelayGadget, Builder);
}
