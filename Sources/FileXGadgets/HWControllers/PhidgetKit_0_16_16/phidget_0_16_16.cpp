// phidget_0_16_16.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE phidget_0_16_16Dll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("phidget_0_16_16.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(phidget_0_16_16Dll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(phidget_0_16_16Dll);
        // Init plugin
        FXString fName=::FxGetModuleName(phidget_0_16_16Dll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("phidget_0_16_16.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(phidget_0_16_16Dll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(Phidget_0_16_16, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(Phidget_0_16_16, Builder);
}
