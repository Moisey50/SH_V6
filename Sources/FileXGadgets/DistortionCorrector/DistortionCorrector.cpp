...
static AFX_EXTENSION_MODULE DistortionCorrectorDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("DistortionCorrector.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(DistortionCorrectorDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(DistortionCorrectorDll);
        // Init plugin
        FXString fName=::FxGetModuleName(DistortionCorrectorDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("DistortionCorrector.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(DistortionCorrectorDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(DistortionCorrectorGadget, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(DistortionCorrectorGadget, Builder);
