...
static AFX_EXTENSION_MODULE PIPFinderDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("PIPFinder.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(PIPFinderDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(PIPFinderDll);
        // Init plugin
        FXString fName=::FxGetModuleName(PIPFinderDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("PIPFinder.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(PIPFinderDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(PIPFinderGadget, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(PIPFinderGadget, Builder);
