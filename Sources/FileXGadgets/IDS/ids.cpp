// IDS.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE IDS_Dll = { NULL, NULL };
...
{
    TRACE0("IDS.DLL Initializing!\n");
	
    // Extension DLL one-time initialization
    if (!AfxInitExtensionModule(IDS_Dll, hInstance))
      return 0;
    ...
    pThisDll = new CDynLinkLibrary(IDS_Dll);
    // Init plugin
    FXString fName=::FxGetModuleName(IDS_Dll.hModule);
    ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
    strcpy(TVDB400_PLUGIN_NAME, fName);
  }
  else if (dwReason == DLL_PROCESS_DETACH)
  {
    TRACE0("IDS.DLL Terminating!\n");
    // Terminate the library before destructors are called
    AfxTermExtensionModule(IDS_Dll);
  }
  return 1;   // ok
}
...
// Add filter collection to graph builder:
    ...
    REGISTER_RUNTIME_GADGET(IDS, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
    ...
    UNREGISTER_RUNTIME_GADGET(IDS, Builder);
}
