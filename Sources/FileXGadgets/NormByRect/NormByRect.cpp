// HSC.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include "NormByRect.h"
...
static AFX_EXTENSION_MODULE NormByRectDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("NormByRectDll.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(NormByRectDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(NormByRectDll);
        // Init plugin
		FXString fName = ::FxGetModuleName(NormByRectDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("NormByRectDll.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(NormByRectDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(NormByRect, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(NormByRect, Builder);
