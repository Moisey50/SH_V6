// RadialDistr.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include <gadgets\gadbase.h>
#include <gadgets\shkernel.h>
#include "RadialDistr.h"
...
static AFX_EXTENSION_MODULE RadialDistrDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		if (!AfxInitExtensionModule(RadialDistrDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(RadialDistrDll);
        // Init plugin
        FXString fName=::FxGetModuleName(RadialDistrDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("RadialDistr.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(RadialDistrDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(RadialDistr, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(RadialDistr, Builder);
}
