...
static AFX_EXTENSION_MODULE ImageSimulatorDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ImageSimulator.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(ImageSimulatorDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(ImageSimulatorDll);
        // Init plugin
        FXString fName=::FxGetModuleName(ImageSimulatorDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ImageSimulator.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(ImageSimulatorDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(ImageSimulator, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(ImageSimulator, Builder);
...
/////////////////////////////////////////////////////////////////////////////
// ImageSimulator Gadgets help

 helpitem helpitems[]=
{	
   { "ImageSimulator" , IDR_GAUSSGEN_HELP }
};
