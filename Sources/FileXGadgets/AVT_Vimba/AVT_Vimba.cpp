// AVT_Vimba.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE AVT_VimbaDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("AVT_Vimba.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(AVT_VimbaDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(AVT_VimbaDll);
        // Init plugin
        FXString fName=::FxGetModuleName(AVT_VimbaDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("AVT_Vimba.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(AVT_VimbaDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(AVT_Vimba, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
    ...
    UNREGISTER_RUNTIME_GADGET(AVT_Vimba, Builder);
}
