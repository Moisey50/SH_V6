// CoordMap.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE CoordMapDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("CoordMap.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(CoordMapDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(CoordMapDll);
        // Init plugin
        FXString fName=::FxGetModuleName(CoordMapDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("CoordMap.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(CoordMapDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(CoordMapGadget, Builder);
	REGISTER_RUNTIME_GADGET(MapControl, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
  ...
  UNREGISTER_RUNTIME_GADGET( CoordMapGadget , Builder );
  UNREGISTER_RUNTIME_GADGET( MapControl , Builder );
