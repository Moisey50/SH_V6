// FXController.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE FXControllerDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("FXController.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(FXControllerDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(FXControllerDll);
        // Init plugin
        FXString fName=::FxGetModuleName(FXControllerDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("FXController.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(FXControllerDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(FXControllerGadget, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(FXControllerGadget, Builder);
