/******************************************************************************
* Filename: MFUtility.cpp
*
* Description: functions from MFUtility.h moved there
...
* 24 Nov 2021   Moisey Bernstein : function moved to this file (in previous versions were in .h file)
*
*
...
* Potential improvements https://docs.microsoft.com/en-us/windows/win32/medfound/media-type-debugging-code.
*/
std::string GetVideoTypeDescriptionBrief( IMFMediaType* pMediaType , CaptureMode& Mode )
{
  std::string description ;
  ...
  {
    CHECK_HR( pMediaType->GetGUID( MF_MT_SUBTYPE , &Mode.m_Guid ) , "Failed to get video sub type." );
    CHECK_HR( MFGetAttributeSize( pMediaType , MF_MT_FRAME_SIZE , 
      &Mode.m_uiWidth , &Mode.m_uiHeight ) , "Failed to get MF_MT_FRAME_SIZE attribute." );
    CHECK_HR( MFGetAttributeRatio( pMediaType , MF_MT_FRAME_RATE , 
      &Mode.m_uiFPSnum , &Mode.m_uiFPSden ) , "Failed to get MF_MT_FRAME_RATE attribute." );

    description += GetGUIDNameConst( Mode.m_Guid );
    strcpy_s( Mode.m_AsFOURCC , GetGUIDNameConst( Mode.m_Guid ) ) ;
    char * pUnderscorePos = strchr( Mode.m_AsFOURCC , '_' ) ;
    if ( pUnderscorePos )
    {
      if ( strstr( pUnderscorePos + 1 , "RGB24" ) == pUnderscorePos + 1 )
        *(DWORD*) (Mode.m_AsFOURCC) = 0 ;
      else
        *(DWORD*) (Mode.m_AsFOURCC) = *(DWORD*) (pUnderscorePos + 1) ;
      Mode.m_AsFOURCC[4] = 0 ;
      Mode.m_Fourcc = *(DWORD*) (Mode.m_AsFOURCC) ;
    }
    Mode.m_iFPS = (int) (Mode.m_uiFPSnum / Mode.m_uiFPSden) ;
    description += ", " + std::to_string( Mode.m_uiWidth ) + 
  ...
  vector<string> * pAsStrings )
{
  ...
  for ( DWORD i = 0; i < mediaTypeCount; i++ )
  {
    ...
    string out = GetMediaTypeDescription( pMediaType ) ;
    std::cout << out << std::endl;
    if ( pAsStrings )
      pAsStrings->push_back( out ) ;
...
void ListModes( IMFSourceReader* pReader ,
  CameraData& Camera , bool brief /* = false */ )
{
  HRESULT hr = NULL;
  DWORD dwMediaTypeIndex = 0;
  string CaptureModesForList ;
  ...
  while ( SUCCEEDED( hr ) )
  {
    ...
    else if ( SUCCEEDED( hr ) )
    {
      CaptureMode Mode ;
      string out = (!brief) ? 
        GetMediaTypeDescription( pType ) : GetVideoTypeDescriptionBrief( pType , Mode ) ;
      if ( out.size() )
      {
        if ( out.find( "MFVideoFormat_" ) == 0 )
        {
          size_t iPos = out.find( '_' ) ;
          out.erase( 0 , iPos + 1 ) ;
        }
        Camera.m_CaptureModesAsStrings.push_back( out ) ;
        CaptureModesForList += out + ';' ;
        Mode.m_iModeIndexInCamera = dwMediaTypeIndex ;
        Camera.m_CaptureModes.push_back( Mode ) ;
      }
  ...
  if ( !CaptureModesForList.empty() )
    Camera.m_CaptureModesForList = CaptureModesForList ;
}
...
HRESULT GetCaptureDevice( const char * pMyDevicePath , CameraData& Camera)
{
  ...
  if (hr == S_OK)
  {
    ...
    if (hr == S_OK)
    {
      ...
      for (UINT i = 0; (i < deviceCount) && (hrFound != S_OK) ; i++)
      {
        LPWSTR pDevicePath = NULL;
        UINT32 uiDevicePathLength = 0;

        hr = ppDevices[i]->GetAllocatedString(
          MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK,
          &pDevicePath, &uiDevicePathLength);

        if (hr == S_OK)
        {
          pDevicePath[uiDevicePathLength] = 0;
          string DevicePathAsASCII = WSTR_tostring(pDevicePath);
          bool bCompare = (DevicePathAsASCII == pMyDevicePath) ;
          if ( bCompare )
          {
            ...
            if (hr == S_OK)
            {
              ...
              if (hr == S_OK)
              {
                string sName = WSTR_tostring(friendlyName);
                FXString DevicePathAsText(pDevicePath);
                CameraData NewCamera(sName.c_str());
                if (ExtractCameraData(DevicePathAsText, NewCamera, AllUSBDevices))
                {
                  ListModes(pSourceReader, NewCamera, true);
                  GetCameraControls(pMediaSource, NewCamera.m_ControlVector);
                  Camera = NewCamera ;
                  hrFound = S_OK;
                }
              }
...
HRESULT ListCaptureDevices( DeviceType deviceType , CamerasVector& Cameras )
{
  ...
  {
    LPWSTR friendlyName = NULL , pDevicePath = NULL ;
    UINT32 friendlyNameLength = 0 , uiDevicePathLength = 0 ;
    IMFMediaSource* pMediaSource = NULL;
    IMFSourceReader* pSourceReader = NULL;
    ...
    if ( hr == S_OK )
    {
      ...
      if ( hr == S_OK )
      {
        hr = MFCreateSourceReaderFromMediaSource(
          pMediaSource , NULL , &pSourceReader );
        if ( hr == S_OK )
        {
          string sName = WSTR_tostring( friendlyName ) ;
          FXString DevicePathAsText( pDevicePath ) ;
...
HRESULT ListVideoDevicesWithBriefFormat( vector<string> * pAsStrings )
{
  IMFAttributes* pDeviceAttributes = NULL;
  IMFActivate** ppDevices = NULL;
  UINT32 deviceCount = 0;
  ...
  for ( UINT i = 0; i < deviceCount; i++ )
  {
    ...
    string out = WSTR_tostring( friendlyName ) ;
    if ( pAsStrings )
      pAsStrings->push_back( out ) ;
    ...
    CameraData Camera ;
    ListModes( pSourceReader , Camera , true );
...
*/
HRESULT ListAudioOutputDevices( vector<string> * pAsStrings )
{
  HRESULT hr = S_OK;
  ...
  for ( DWORD i = 0; i < deviceCount; i++ )
  {
    ...
    string out = WSTR_tostring( wstrID ) ;
    if ( pAsStrings )
      pAsStrings->push_back( out ) ;
...
HRESULT GetSourceFromCaptureDevice( DeviceType deviceType , 
  UINT nDevice , IMFMediaSource** ppMediaSource , IMFSourceReader** ppMediaReader )
{
  ...
  BYTE* bitmapData , DWORD bitmapDataLength )
{
  ...
  file = CreateFile( fileName , GENERIC_WRITE , 
    0 , NULL , CREATE_ALWAYS , FILE_ATTRIBUTE_NORMAL , NULL );  //Sets up the new bmp to be written to
...
void CreateBitmapFromSample( LPCSTR fileName , 
  ...
  WORD bitsPerPixel , IMFSample* pSample )
{
...
HRESULT WriteSampleToFile( IMFSample* pSample , std::ofstream* pFileStream )
{
  ...
  if ( hr != S_OK )
  {
    pErrMsg = "ConvertToContiguousBuffer failed." ;
  }
  else
  {
    ...
    if ( hr != S_OK )
    {
      pErrMsg = "Get buffer length failed." ;
    }
    else
    {
      TRACE( "Writing sample to capture file sample size %i.\n" , bufLength );
  ...
  IMFSample* pSrcSample , IMFSample** pDstSample )
{
  ...
  CHECK_HR_SH( hr , "Failed to get total length from source buffer." );

  hr = CreateSingleBufferIMFSample( srcBufLength , pDstSample );
  CHECK_HR_SH( hr , "Failed to create new single buffer IMF sample." );

  hr = pSrcSample->CopyAllItems( *pDstSample );
  CHECK_HR_SH( hr , "Failed to copy IMFSample items from src to dst." );

  hr = (*pDstSample)->GetBufferByIndex( 0 , &pDstBuffer );
  CHECK_HR_SH( hr , "Failed to get buffer from sample." );

  hr = pSrcSample->CopyToBuffer( pDstBuffer );
  CHECK_HR_SH( hr , "Failed to copy IMF media buffer." );
  ...
  IMFSample** pOutSample , BOOL* transformFlushed )
{
  ...
  *transformFlushed = FALSE;
  ...
  CHECK_HR_SH( hr , "Failed to get output stream info from MFT." );
...

  {
    ...
    {
      // Format of the input stream has changed. 
      ...
      if ( outputDataBuffer.dwStatus == MFT_OUTPUT_DATA_BUFFER_FORMAT_CHANGE )
      {
        ...
        CHECK_HR_SH( hr , "Failed to get the MFT output media type after a stream change." );

        std::cout << "MFT output media type: " <<
          GetMediaTypeDescription( pChangedOutMediaType ) << std::endl << std::endl;

        hr = pChangedOutMediaType->SetGUID( MF_MT_SUBTYPE , MFVideoFormat_IYUV );
        CHECK_HR_SH( hr , "Failed to set media sub type." );

        hr = pTransform->SetOutputType( 0 , pChangedOutMediaType , 0 );
        CHECK_HR_SH( hr , "Failed to set new output media type on MFT." );

        hr = pTransform->ProcessMessage( MFT_MESSAGE_COMMAND_FLUSH , NULL );
        CHECK_HR_SH( hr , "Failed to process FLUSH command on MFT." );

        *transformFlushed = TRUE;
      }
      else
      {
        printf( "MFT stream changed but didn't have"
          " the data format change flag set. Don't know what to do.\n" );
        hr = E_NOTIMPL;
      }
...
/* Utility for camera or amplifier control description forming
   and append to string array
*/
bool PushControl( IMFMediaSource* pMediaSource , const char * pControlGroup , 
  const char * pControlName , int ControlId , 
  ControlVector& CVector )
{
  CaptureControl NewControl ;
  HRESULT hr = S_FALSE ;
  if ( strcmp( pControlGroup , "CamControl" ) == 0 )
  {
    CComQIPtr<IAMCameraControl> spCameraControl( pMediaSource );
    if ( spCameraControl )
    {
      hr = spCameraControl->GetRange( (CameraControlProperty) ControlId ,
        &NewControl.m_lMin , &NewControl.m_lMax , 
        &NewControl.m_lStep , &NewControl.m_lDefault , &NewControl.m_lFlags );
      if ( hr == S_OK )
      {
        spCameraControl->Get( (CameraControlProperty) ControlId ,
          &NewControl.m_lCurrentValue , &NewControl.m_lCurrentFlags ) ;
        NewControl.m_Node = CN_Camera ;
      }
    }
  }
  else   if ( strcmp( pControlGroup , "AmpControl" ) == 0 )
  {
    CComQIPtr<IAMVideoProcAmp> spVideoAmp( pMediaSource );
    if ( spVideoAmp )
    {
      hr = spVideoAmp->GetRange( (VideoProcAmpProperty) ControlId ,
        &NewControl.m_lMin , &NewControl.m_lMax ,
        &NewControl.m_lStep , &NewControl.m_lDefault , &NewControl.m_lFlags );
      if ( hr == S_OK )
      {
        spVideoAmp->Get( (VideoProcAmpProperty) ControlId ,
          &NewControl.m_lCurrentValue , &NewControl.m_lCurrentFlags ) ;
        NewControl.m_Node = CN_Amplifier ;
      }
    }
  }
  if ( (hr == S_OK) && (NewControl.m_lMax != NewControl.m_lMin))
  {
    strcpy_s( NewControl.m_Name , pControlName ) ;
    NewControl.m_Prop = ControlId ;
    CVector.push_back( NewControl ) ;
    return true ;
  }
  return false ;
}
...
{
  int iInitialSize = (int) CVector.size() ;
  CamControlDef Controls[] = {
    { "CamControl" , "Exposure" , CameraControl_Exposure } ,
    { "CamControl" , "Focus" ,   CameraControl_Focus } ,
    { "AmpControl" , "Brightness" , VideoProcAmp_Brightness } ,
    { "AmpControl" , "Contrast" , VideoProcAmp_Contrast } ,
    { "AmpControl" , "Gain" , VideoProcAmp_Gain } ,
    { "AmpControl" , "Gamma" , VideoProcAmp_Gamma } ,
    { "AmpControl" , "WhiteBalance" , VideoProcAmp_WhiteBalance } ,
    { "AmpControl" , "Hue" , VideoProcAmp_Hue } ,
    { "AmpControl" , "Saturation" , VideoProcAmp_Saturation } ,
    { "AmpControl" , "Sharpness" , VideoProcAmp_Sharpness } ,
    { "AmpControl" , "ColorEnable" , VideoProcAmp_ColorEnable } ,
    { "AmpControl" , "BackCompens" , VideoProcAmp_BacklightCompensation } ,
    { "CamControl" , "Pan" , CameraControl_Pan } ,
    { "CamControl" , "Tilt" , CameraControl_Tilt } ,
    { "CamControl" , "Roll" , CameraControl_Roll } ,
    { "CamControl" , "Zoom" , CameraControl_Zoom } ,
    { "CamControl" , "Iris" , CameraControl_Iris } 
  } ;

  for ( int i = 0 ; i < sizeof(Controls)/sizeof(Controls[0]) ; i++ )
  {
    PushControl( pMediaSource , Controls[ i ].pGroupName ,
      Controls[ i ].pControlName , Controls[ i ].id , CVector ) ;
  }
  return (int) CVector.size() - iInitialSize ;
}
