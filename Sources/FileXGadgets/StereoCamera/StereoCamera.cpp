...
static AFX_EXTENSION_MODULE StereoCameraDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("StereoCamera.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(StereoCameraDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(StereoCameraDll);
        // Init plugin
        FXString fName=::FxGetModuleName(StereoCameraDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("StereoCamera.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(StereoCameraDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(StereoCameraGadget, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(StereoCameraGadget, Builder);
