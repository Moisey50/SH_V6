// FRenderDll.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE FRenderDllDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("FRenderDll.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(FRenderDllDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(FRenderDllDll);
        // Init plugin
        FXString fName=::FxGetModuleName(FRenderDllDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("FRenderDll.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(FRenderDllDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
  ...
  REGISTER_RUNTIME_GADGET(FRender, Builder);
...
{
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(FRender, Builder);
