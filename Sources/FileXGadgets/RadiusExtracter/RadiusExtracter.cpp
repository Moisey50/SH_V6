...
static AFX_EXTENSION_MODULE UserExampleDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("RadiusExtracter.cpp Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(UserExampleDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(UserExampleDll);
        // Init plugin
        FXString fName=::FxGetModuleName(UserExampleDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("RadiusExtracter.cpp Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(UserExampleDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(RadiusExtracterGadget, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(RadiusExtracterGadget, Builder);
