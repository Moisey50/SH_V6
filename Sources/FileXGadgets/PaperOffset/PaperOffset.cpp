// PaperOffset.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include <gadgets\gadbase.h>
#include <gadgets\shkernel.h>
#include "PaperOffset.h"
...
static AFX_EXTENSION_MODULE PaperOffset_Dll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
  ...
  {
    TRACE0("PaperOffset.DLL Initializing!\n");

    // Extension DLL one-time initialization
    if (!AfxInitExtensionModule(PaperOffset_Dll, hInstance))
      return 0;
    ...
    pThisDll = new CDynLinkLibrary(PaperOffset_Dll);
    // Init plugin
    FXString fName=::FxGetModuleName(PaperOffset_Dll.hModule);
    ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
    strcpy(TVDB400_PLUGIN_NAME, fName);
  }
  else if (dwReason == DLL_PROCESS_DETACH)
{
    TRACE0("PaperOffset.DLL Terminating!\n");
    // Terminate the library before destructors are called
    AfxTermExtensionModule(PaperOffset_Dll);
}
  return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(PaperOffset, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
    UNREGISTER_RUNTIME_GADGET(PaperOffset, Builder);
}
