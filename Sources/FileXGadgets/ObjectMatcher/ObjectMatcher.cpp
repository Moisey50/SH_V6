...
static AFX_EXTENSION_MODULE ObjectMatcherDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ObjectMatcher.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(ObjectMatcherDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(ObjectMatcherDll);
        // Init plugin
        FXString fName=::FxGetModuleName(ObjectMatcherDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ObjectMatcher.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(ObjectMatcherDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(ObjectMatcherGadget, Builder);
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(ObjectMatcherGadget, Builder);
