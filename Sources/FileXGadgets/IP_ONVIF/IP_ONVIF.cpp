// IP_Onvif.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE IP_OnvifDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("IP_Onvif.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(IP_OnvifDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(IP_OnvifDll);
        // Init plugin
        FXString fName=::FxGetModuleName(IP_OnvifDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("IP_Onvif.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(IP_OnvifDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(IP_OnvifCam, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
    ...
    UNREGISTER_RUNTIME_GADGET(IP_OnvifCam, Builder);
}
