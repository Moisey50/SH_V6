// SoliosDalsa.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE SoliosDalsaDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
  ...
  {
    TRACE0("SoliosDalsa.DLL Initializing!\n");

    // Extension DLL one-time initialization
    if (!AfxInitExtensionModule(SoliosDalsaDll, hInstance))
      return 0;
    ...
    pThisDll = new CDynLinkLibrary(SoliosDalsaDll);
    // Init plugin
    FXString fName=::FxGetModuleName(SoliosDalsaDll.hModule);
    ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
    strcpy(TVDB400_PLUGIN_NAME, fName);
  }
  else if (dwReason == DLL_PROCESS_DETACH)
  {
    TRACE0("SoliosDalsa.DLL Terminating!\n");
    // Terminate the library before destructors are called
    AfxTermExtensionModule(SoliosDalsaDll);
  }
  return 1;   // ok
}
...
// Add filter collection to graph builder:
    ...
    REGISTER_RUNTIME_GADGET(SoliosDalsa, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
    ...
    UNREGISTER_RUNTIME_GADGET(SoliosDalsa, Builder);
}
...
// helpitem helpitems[]=
// {	
//     {"SoliosDalsa",IDR_SoliosDalsa1_help}
// };
// 
