// BrightnessAverage.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include "BrightnessAverage.h"
...
static AFX_EXTENSION_MODULE BrightnessAverageDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("BrightnessAverage.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(BrightnessAverageDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(BrightnessAverageDll);
        // Init plugin
        FXString fName=::FxGetModuleName(BrightnessAverageDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("BrightnessAverage.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(BrightnessAverageDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
  ...
  REGISTER_RUNTIME_GADGET( BrightnessAverageGadget , Builder );
  REGISTER_RUNTIME_GADGET( MultiAver , Builder );
  REGISTER_RUNTIME_GADGET( CalcDiffSum , Builder );
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(BrightnessAverageGadget, Builder);
  UNREGISTER_RUNTIME_GADGET( MultiAver , Builder );
  UNREGISTER_RUNTIME_GADGET( CalcDiffSum , Builder );
}
