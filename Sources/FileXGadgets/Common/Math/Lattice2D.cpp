...
cLattice2D::cLattice2D(const ns::R2& origin_, const ns::R2& e0_, const ns::R2& e1_)
    :origin(origin_), e0(e0_), e1(e1_)
{
    double e0e0 = ns::R2::dot(e0, e0);
    double e0e1 = ns::R2::dot(e0, e1);
    double e1e1 = ns::R2::dot(e1, e1);
...
double cLattice2D::DistanceToNode(const int i0, int i1, const ns::R2& r) const
{
    double ddx = origin.x + i0 * e0.x + i1 * e1.x - r.x;
    double ddy = origin.y + i0 * e0.y + i1 * e1.y - r.y;
    return ddx * ddx + ddy * ddy;

    ns::R2 dr{ origin.x - r.x, origin.y - r.y }, r0;
...
bool cLattice2D::NearestNode(const ns::R2& r, int& i0, int& i1) const
{
    ns::R2 dr{ origin.x - r.x, origin.y - r.y }, r0;
...
void cLattice2D::ToLatticeCoordinates(const ns::R2& r, ns::R2& lt, int& i0, int& i1)
{
    ns::R2 dr{ r.x - origin.x, r.y - origin.y };
...
void cLattice2D::FromLatticeCoordinates(const ns::R2& lt, ns::R2& r)
{
