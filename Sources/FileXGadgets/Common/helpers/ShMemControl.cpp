...
#ifdef TRACE_SH_MEM_EXCHANGE
  TRACE( "WaitAndReceiveAnswer: hThr=0x%08X Res=%d Inside=%8.3f %s\n" , GetCurrentThreadId() , iResult , get_current_time() - dEntryTime , GetShMemAreaName() ) ;
#endif
  return iResult ; // timeout
};
  ...
  void * pBuf , int iMsgLen , int iTimeout_ms , int iBufLen ) 
{
...
#ifdef TRACE_SH_MEM_EXCHANGE
  double dEntryTime = get_current_time() ;
  TRACE( "SendRqWaitAndReceiveAnswer: hThr=0x%08X Entry                   %s\n" , GetCurrentThreadId() , GetShMemAreaName() ) ;
#endif
  int iResult = 0 ;
  ResetOutEvent() ;
  ResetInEvent() ;
  ...
  iBufLen += 10 ;
    ...
    if ( !iResult ) //timeout
    {
      TRACE( "SndRcv %s: hT=0x%08X Can't Rcv\n" , GetShMemAreaName() , GetCurrentThreadId()  ) ;
    }
  }
  else
  {
    TRACE( "SndRcv %s: hT=0x%08X Can't Send Rq\n" , GetShMemAreaName() , GetCurrentThreadId() ) ;
  }
#ifdef TRACE_SH_MEM_EXCHANGE
  TRACE( "SendRqWaitAndReceiveAnswer: hThr=0x%08X Res=%d Inside=%8.3f %s\n" , GetCurrentThreadId() , iResult , get_current_time() - dEntryTime , GetShMemAreaName() ) ;
#endif
  m_Lock.Unlock() ;
  return iResult ;
};
