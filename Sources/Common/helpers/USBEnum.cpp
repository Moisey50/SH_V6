...
#define THIS_MODULENAME ("USBEnum")
...
int g_RootHubIndex = 0 ;
int g_SubHubIndex = 0 ;
__int64 g_CurrentIndex = 0L ; //this is hub sequence index: root hub index is left one and each following
                         // lead to multiply this number by 100 and add current port number
                         // Such approach provides warranty, that new hub or device connection
                         // does not change another devices index and port number
int g_PortIndex = 0 ;

char g_Devices[ 60000 ] ;
int  g_CurrentLen ;

char g_szCurrentHubLocation[1000];

const char * GetDevices() { return g_Devices ; }

int AddDevice( const char * pNewDevice )
{
  strcat_s( g_Devices , sizeof( g_Devices ) , pNewDevice ) ;
  strcat_s( g_Devices , sizeof( g_Devices ) , ";\n" ) ;
  return (int)strlen( g_Devices ) ;
}
...
{
   (PCHAR)"NoDeviceConnected" ,  // 0  - NoDeviceConnected
   (PCHAR)"DeviceConnected" ,   // 1  - DeviceConnected
   (PCHAR)"FailedEnumeration" , // 2  - DeviceFailedEnumeration
   (PCHAR)"GeneralFailure" ,    // 3  - DeviceGeneralFailure
   (PCHAR)"Overcurrent" ,       // 4  - DeviceCausedOvercurrent
   (PCHAR)"NotEnoughPower" ,    // 5  - DeviceNotEnoughPower
   (PCHAR)"NotEnoughBandwidth" ,// 6  - DeviceNotEnoughBandwidth
   (PCHAR)"HubNestedTooDeeply" ,// 7  - DeviceHubNestedTooDeeply
   (PCHAR)"InLegacyHub" ,       // 8  - DeviceInLegacyHub
   (PCHAR)"Enumerating" ,       // 9  - DeviceEnumerating
   (PCHAR)"Reset"               // 10 - DeviceReset
  }; 
...
)
{
  ...
  //
  hcInfo = (PUSBHOSTCONTROLLERINFO) malloc( sizeof( USBHOSTCONTROLLERINFO ) );
...
//     }
  ...
  if ( rootHubName != NULL )
  {
    ...
    {
      g_SubHubIndex++ ;
      char szSavedHubLocation[1000];
      strcpy_s(szSavedHubLocation, sizeof(szSavedHubLocation), g_szCurrentHubLocation);
      ...
      strcpy_s(g_szCurrentHubLocation, sizeof(g_szCurrentHubLocation), szSavedHubLocation);
    }
  }
  else
  {
...
)
{
  ...
  if (PortConnectorProps != NULL)
  {
    sprintf_s(g_szCurrentHubLocation + strlen(g_szCurrentHubLocation),
      sizeof(g_szCurrentHubLocation), ".%d", PortConnectorProps->ConnectionIndex);
  }
  // Allocate some space for a USBDEVICEINFO structure to hold the
  // hub info, hub name, and connection info pointers.  GPTR zero
  // initializes the structure for us.
  //
  info = malloc( sizeof( USBEXTERNALHUBINFO ) );
  if ( info == NULL )
  {
    OOPS();
  ...
  deviceName = (PCHAR) malloc( (DWORD) cchFullHubName );
  if ( deviceName == NULL )
  {
    OOPS();
    goto EnumerateHubError;
...
)
{
  ...
  CHAR        leafName[ 512 ];
  int         icon = 0;
  int         iNChildren = 0 , iNAllChildren = 0 ;
  ...
  for ( index = 1; index <= NumPorts; index++ )
  {
    ...
    if ( success && nBytes == sizeof( USB_PORT_CONNECTOR_PROPERTIES ) )
    {
      pPortConnectorProps = (PUSB_PORT_CONNECTOR_PROPERTIES)
        malloc( portConnectorProps.ActualLength );
    ...
    if ( connectionInfoEx->DeviceIsHub )
    {
      ...
      if ( extHubName != NULL )
      {
        ...
        if ( SUCCEEDED( hr ) )
        {
          ...
          char szSavedHubLocation[1000];
          strcpy_s(szSavedHubLocation, sizeof(szSavedHubLocation), g_szCurrentHubLocation);
          __int64 iCurrentIndex = g_CurrentIndex ;
          g_CurrentIndex = g_CurrentIndex * 100 + g_PortIndex ;
...
//           cchDeviceDesc );
//         char szHubs[ 100 ] ;
//         sprintf_s( szHubs , sizeof( szHubs ) , "/R%d.H%d.P%d/" ,
//           g_RootHubIndex , g_SubHubIndex , g_PortIndex ) ;
//         strcat_s( (char*) leafName , sizeof( leafName ) , szHubs ) ;
//         strcat_s( (char*) leafName , sizeof( leafName ) , (char*) DevProps->DeviceId ) ;
//         g_CurrentLen = AddDevice( leafName ) ;
//       }
      ...
      if ( DevProps )
      {
        ...
        StringCchCatN( leafName ,
          ...
          DevProps->DeviceDesc ,
          cchDeviceDesc );
        __int64 i64Index = g_CurrentIndex ;
        __int64 i64Divider = 1000000000000000000L; 
        __int64 i64Rest = i64Index / i64Divider ;
        char szHubs[ 100 ] ;
        while ( (i64Index < i64Divider) && (i64Divider > 0) )
          i64Divider /= 100 ;
        if ( i64Divider > 0 )
        {
          int iPos = 0 ;
          do 
          {
            int iHub = ( int ) ( i64Index / i64Divider ) ;
            sprintf_s( szHubs + iPos , sizeof( szHubs ) - iPos - 1 ,
              "%d." , iHub ) ;
            iPos = (int)strlen( szHubs ) ;
            i64Index -= iHub * i64Divider ;
            i64Divider /= 100 ;
          } while ( i64Divider >= 100 );
          if ( i64Index > 0 )
          {
            sprintf_s( szHubs + iPos , sizeof( szHubs ) - iPos - 1 ,
              "%d." , ( int ) i64Index ) ;
          ...
          char szLocation[ 200 ] ;
          sprintf_s( szLocation, sizeof( szLocation ), "/R%sP%d/",
            szHubs , g_PortIndex);
          strcat_s((char*)leafName, sizeof(leafName), szLocation );
          strcat_s((char*)leafName, sizeof(leafName), (char*)DevProps->DeviceId);
          AddDevice( leafName  );
...
//         strcat_s( (char*) leafName , sizeof( leafName ) , (char*) DevProps->DeviceId ) ;
      ...
      if ( pNode && pNode->DeviceInfoData.DevInst )
      {
        iNChildren = EnumerateChildren( pNode->DeviceInfoData.DevInst ,
          hTreeParent , (LPARAM) info , (TREEICON) icon ) ;
        iNAllChildren += iNChildren ;
      }
    }
  } // for
}
...
)
{
  ...
  pWideStr = (PWCHAR) malloc( (DWORD) cbWideStr + sizeof( WCHAR ) );
  if ( NULL == pWideStr )
  {
    return NULL;
  }
...
)
{
  ...
  bosDescReq = (PUSB_DESCRIPTOR_REQUEST) malloc( nBytes );
  ...
  PUSBHOSTCONTROLLERINFO hcInfo )
{
  ...
  {
    hcInfo->ControllerInfo = (PUSB_CONTROLLER_INFO_0) malloc( sizeof( USB_CONTROLLER_INFO_0 ) );
...
{
  int iNChildren = 0 ;
  DEVINST FirstChildInst = 0 , OtherChildInst = 0 ;
  CONFIGRET bRes = CM_Get_Child( &FirstChildInst , DevInst , 0 ) ;
  if ( bRes == CR_SUCCESS )
  {
    ULONG ChildIdSize = 0 ;
    bRes = CM_Get_Device_ID_Size( &ChildIdSize , FirstChildInst , 0 ) ;
    if ( bRes == CR_SUCCESS )
    {
      char ChildId[ 4000 ] , leaf[ 1000 ] ;
      bRes = CM_Get_Device_ID( FirstChildInst , ChildId , sizeof( ChildId ) , 0 ) ;
      if ( bRes == CR_SUCCESS )
      {
        sprintf_s(leaf, sizeof(leaf), "   Child Device /R%d%s.P%d/",
          g_RootHubIndex, g_szCurrentHubLocation, g_PortIndex);
        strcat_s(leaf, sizeof(leaf), ChildId);
        AddDevice( leaf );
        iNChildren++ ;
        while ( CM_Get_Sibling( &OtherChildInst , FirstChildInst , 0 ) == CR_SUCCESS )
        {
          bRes = CM_Get_Device_ID_Size( &ChildIdSize , OtherChildInst , 0 ) ;
          if ( bRes == CR_SUCCESS )
          {
            bRes = CM_Get_Device_ID( OtherChildInst , ChildId , sizeof( ChildId ) , 0 ) ;
            if ( bRes == CR_SUCCESS )
            {
              sprintf_s(leaf, sizeof(leaf), "   Child Device /R%d%s.P%d/",
                g_RootHubIndex, g_szCurrentHubLocation, g_PortIndex);
...
)
{
  ...
  if ( DeviceList->DeviceInfo != INVALID_HANDLE_VALUE )
  {
    ...
    while ( error != ERROR_NO_MORE_ITEMS )
    {
      ...
      memset( pNode , 0 , sizeof( DEVICE_INFO_NODE ) ) ;
...
FXString GetAllUSBDevices()
{
  ...
  ULONG ulLastEnumDevicesNumber ; // not only Video!!!  
  EnumerateHostControllers( NULL , &ulLastEnumDevicesNumber ) ;
