...
__forceinline pTVFrame _FeatureDetector(const pTVFrame frame)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
/*
        ...
        int eq = _equal_(*ss,*(ss-1)) + equal(*ss,*(ss+1)) + equal(*ss,*(ss-width)) + equal(*ss,*(ss+width))
              +  _equal_(*ss,*(ss-width-1)) + equal(*ss,*(ss+width-1)) + equal(*ss,*(ss-width+1)) + equal(*ss,*(ss+width+1));
