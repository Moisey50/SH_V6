  ...
  case BI_Y8:
    {
      ...
      {
        int val=*fPntr;
        int deltaBW=(int)(*wPntr-*bPntr);
        if (deltaBW<=0)
          deltaBW=1;
        int deltaVB = (val-*bPntr) ;
        if ( deltaVB <= 0 )
          val = 255 / deltaBW ;
        else
          val=(deltaVB*255)/deltaBW;
...
__forceinline void _normalize_bwt(pTVFrame frame, pTVFrame black, pTVFrame white , int iTarget_perc = 94 )
{
  ASSERT(frame);
  ASSERT(frame->lpBMIH);
  ...
  case BI_Y8:
    {
      int iTarget = (255 * iTarget_perc)/100 ;
      LPBYTE fPntr=GetData(frame),wPntr=GetData(white), bPntr=GetData(black);
      LPBYTE eod=fPntr+GetI8Size(frame);
      while (fPntr<eod)
      {
  ...
  case BI_Y16:
    {
      int iTarget = (65535 * iTarget_perc)/100 ;
      LPWORD fPntr=(LPWORD)GetData(frame),wPntr=(LPWORD)GetData(white), bPntr=(LPWORD)GetData(black);
      LPWORD eod=fPntr+GetI8Size(frame);
      while (fPntr<eod)
      {
...
__forceinline void _normalize_wt(pTVFrame frame, pTVFrame white , int iTarget_perc = 94 )
{
  ASSERT(frame);
  ASSERT(frame->lpBMIH);
...
__forceinline int avgint_4(LPBYTE off, int w)
{
  LPBYTE o=off;
  int a;
  a=*(o) + *(o+1) + *(o+2)+ *(o+3);
...
__forceinline int avgint_2(LPBYTE off, int w)
{
  LPBYTE o=off;
  int a;
  a=*(o) + *(o+1);
...
__forceinline void colorbalance2_YUV12(LPBYTE data, int width, int height)
{
  LPBYTE U,V,EOD;
  int umedian=0,vmedian=0;
  int size=width*height;
...
__forceinline void colorbalance(pTVFrame frame)
{
  ...
  if (frame->lpBMIH->biCompression==BI_YUV12)
    colorbalance2_YUV12(GetData(frame), frame->lpBMIH->biWidth, frame->lpBMIH->biHeight);
}

__forceinline BYTE _shift(int val, int shift)
{
  val+=shift;
  val=(val<0)?0:(val>255)?255:val;
  return (BYTE)val;
...
__forceinline void colorshiftYUV9(LPBYTE data, int width, int height, int us, int vs)
{
  LPBYTE U,V,EOD;
  int size=width*height;
  ASSERT(size%16==0);
...
__forceinline void colorshiftYUV12(LPBYTE data, int width, int height, int us, int vs)
{
  LPBYTE U,V,EOD;
  int size=width*height;
  ASSERT(size%4==0);
...
{
  if (frame->lpBMIH->biCompression==BI_YUV9)
    colorshiftYUV9(GetData(frame), frame->lpBMIH->biWidth, frame->lpBMIH->biHeight, U, V);
  else if (frame->lpBMIH->biCompression==BI_YUV12)
    colorshiftYUV12(GetData(frame), frame->lpBMIH->biWidth, frame->lpBMIH->biHeight, U, V);
}
...
__forceinline void _minus_black(pTVFrame frame,pTVFrame black , int iOffset = 0 )
{
  ASSERT(frame);
  ASSERT(frame->lpBMIH);
  ...
  case BI_Y8:
    {
      ...
      while (fPntr<eod)
      {
        int iVal = ( int )( *fPntr ) - ( int )( *bPntr ) + iOffset ;
        *fPntr = (BYTE)((iVal > 0) ? ((iVal <= 255)? iVal : 255 ) : 0) ;
        fPntr++; bPntr++;
      }
    }           
    break;
  case BI_Y16:
    {
      ...
      {
        int iVal = ( int )( *fPntr ) - ( int )( *bPntr ) + iOffset ;
        *fPntr = (WORD)(( iVal > 0 ) ? ( ( iVal <= 65535 ) ? iVal : 65535 ) : 0) ;
        fPntr++; bPntr++;
      }
    }           
  }
