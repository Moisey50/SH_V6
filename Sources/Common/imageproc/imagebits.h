...
__forceinline bool __verifyInFrame( pTVFrame frame , POINT pnt )
{
  bool res = true;
...
__forceinline bool __verifyInFrame( pTVFrame frame , RECT rc )
{
  bool res = true;
...
__forceinline LPBYTE __getdata_I_XY( const pTVFrame frame , int X , int Y )
{
...
__forceinline unsigned getdata_I_XY( const pTVFrame frame , int X , int Y )
{
  int offset = X + Y * (frame->lpBMIH->biWidth);
...
__forceinline bool __getdata_IUV( const pTVFrame frame , int X , int Y , int& I , int& U , int& V )
{
  if ( !frame
    || (X < 0) || (Y < 0)
    || (X >= frame->lpBMIH->biWidth)
    || (Y >= frame->lpBMIH->biHeight) )
    return false ;

  LPBYTE src = (frame->lpData) ? frame->lpData : (((LPBYTE) frame->lpBMIH) + frame->lpBMIH->biSize);
  int offset = X + Y * (frame->lpBMIH->biWidth);
  int colWidth = (frame->lpBMIH->biWidth >> 1); // YUV12
...
__forceinline bool __setdata_IUV( pTVFrame frame , int X , int Y , int I , int U , int V )
{
  ...
  int offset = X + Y * (frame->lpBMIH->biWidth);
  int colWidth = (frame->lpBMIH->biWidth >> 1); // BI_YUV12
  int ISize = frame->lpBMIH->biWidth*frame->lpBMIH->biHeight;
...
__forceinline bool __getdata_RGB( pTVFrame frame , int X , int Y , int& R , int& G , int& B )
{
  if ( !frame
    || (X < 0) || (Y < 0)
    || (X >= frame->lpBMIH->biWidth)
    || (Y >= frame->lpBMIH->biHeight)
    || (frame->lpBMIH->biCompression != BI_RGB) )
    return false ;

  int iWidth = frame->lpBMIH->biWidth ;
  int iHeight = frame->lpBMIH->biHeight ;
  int ISize = iWidth * iHeight * 3;
  LPBYTE src = (frame->lpData) ? frame->lpData
    : (((LPBYTE) frame->lpBMIH) + frame->lpBMIH->biSize);
  int offset = (X + (iHeight - Y - 1) * iWidth) * 3  ;
  src += offset ;
  B = *(src++) ;
  G = *(src++) ;
  R = *(src) ;
