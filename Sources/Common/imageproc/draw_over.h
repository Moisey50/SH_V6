...
__forceinline pTVFrame _draw_over(const pTVFrame bgFrame, const pTVFrame fgFrame, int x, int y)
{
	pTVFrame Copy=NULL;
    if ((bgFrame->lpBMIH->biCompression==BI_YUV9) || (bgFrame->lpBMIH->biCompression==BI_Y8)
      || (bgFrame->lpBMIH->biCompression == BI_Y800) || (bgFrame->lpBMIH->biCompression == BI_Y12) 
      || ( bgFrame->lpBMIH->biCompression == BI_YUV12 ) )
