...
__forceinline pTVFrame _lpass(const pTVFrame frame, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
__forceinline pTVFrame _hpass(const pTVFrame frame, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
__forceinline pTVFrame _8_1_passfilter(const pTVFrame frame)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
    ...
    case BI_YUV12:
        {
            ...
            memset(dst+width*depth,128,(width*depth)/2);
...
__forceinline pTVFrame _lpass_1DH(const pTVFrame frame, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
__forceinline pTVFrame _lpass_1DV( const pTVFrame frame, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
__forceinline pTVFrame _hpass_1DH(const pTVFrame frame, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
__forceinline pTVFrame _hpass_1DV(const pTVFrame frame, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
__forceinline pTVFrame _sharpen(const pTVFrame frame, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ASSERT(frame);
    ASSERT(frame->lpBMIH);
...
__forceinline LPBITMAPINFOHEADER  _sharpen(LPBITMAPINFOHEADER bmih, int __fr_nmb=CONOVOLUTION_PARAM)
{
    ...
    pTVFrame dst=_sharpen(&src, __fr_nmb);
...
typedef enum { box3x3=0, box5x5=1, box7x7=2 } boxsize;

__forceinline pTVFrame _sqrfilter8(const pTVFrame frame, boxsize box)
{
    ...
    else if (frame->lpBMIH->biCompression==BI_YUV12) // copy color info
    {
...
__forceinline pTVFrame _sqrfilter16(const pTVFrame frame, boxsize box)
{
    pTVFrame retV=(pTVFrame)malloc(sizeof(TVFrame)); memset(retV,0,sizeof(TVFrame));
    retV->lpBMIH=(LPBITMAPINFOHEADER)malloc(getsize4BMIH(frame));
    memcpy(retV->lpBMIH,frame->lpBMIH,frame->lpBMIH->biSize);
...
__forceinline pTVFrame _sqrfilter(const pTVFrame frame, boxsize box)
{
