...
int CSegmentation::FindSpots( pTVFrame pFrame , BOOL bDetailed )
{
  ...
  for (int iSegm = 0 ; iSegm < m_pCurr->GetSize() ; iSegm++ )
  {
    ...
    if ( m_pCurr->ElementAt(iSegm).m_Segm.m_iB > 0 )
      NewSpot.m_iBefore = GetPixel( pFrame , 
        m_pCurr->ElementAt(iSegm).m_Segm.m_iB - 1 , m_pCurr->ElementAt(iSegm).m_iY ) ;
    NewSpot.m_iAfter = GetPixel( pFrame , 
      m_pCurr->ElementAt(iSegm).m_Segm.m_iB  , m_pCurr->ElementAt(iSegm).m_iY ) ;
  ...
  for (int iY = 1 ; iY < m_iHeight ; iY++ )
  {
    ...
    for (int iCurrNew = 0 ; iCurrNew < iCurrLen ; iCurrNew++)
    {
      if (pCurr[iCurrNew].m_iMatched == 0)
      {
        ...
        if ( m_pCurr->ElementAt(iCurrNew).m_Segm.m_iB > 0 )
          NewSpot.m_iBefore = GetPixel( pFrame , 
          m_pCurr->ElementAt(iCurrNew).m_Segm.m_iB - 1 , m_pCurr->ElementAt(iCurrNew).m_iY ) ;
        NewSpot.m_iAfter = GetPixel( pFrame , 
          m_pCurr->ElementAt(iCurrNew).m_Segm.m_iB  , m_pCurr->ElementAt(iCurrNew).m_iY ) ;
        m_ColSpots.Add(NewSpot) ;
      }
    }
