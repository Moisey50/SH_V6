...
public:
	...
	public:
		...
		void SetSpan( int iSpan ) { m_iColSpan = iSpan; }
		int GetSpan() { return m_iColSpan; }
	...
	bool Create( const char* szFile, bool bInterLocked=false, 
	...
	bool CreateTable( int iColumns, char* szCaption=0, int iBorder=1, 
					...
					int iCellPad=0, int iCellSpace=0 );
	bool CloseTable();

  void ResetAllColumns();
	bool SetColumn( int iColumn, const char* szData );
	bool SetColumnWidth( int iColumn, int iWidth, bool bAbs=false );
	bool AddLine( CLogFont* pFont=NULL, ELineType lt=LT_Line, EAlign al=A_Left );
	//<hr noshade size=1 width=100% color=cccccc>
	bool AddHR( int iSize=1, int iWidth=100, bool bShade=false, CLogColor colorHR=0 );
	bool AddBR( int iCount=1 );
	...
	bool AddComment( const char* szComment );

  uint GetTotalLinesNumber() { return m_uiTotalLineNumber; }
...
protected:
	...
	bool			m_bNewFile;
	CGenFile		m_File;
	uint			m_uiLineNumber;  //reset for each table
  uint      m_uiTotalLineNumber;   //for whole protocol
	CLock*			m_pLock;
};
