...
CDebugWnd::~CDebugWnd()
{
	if (m_pHostWnd && ::IsWindow(m_pHostWnd->GetSafeHwnd()))
		m_pHostWnd->SendMessage(WM_PARENTNOTIFY, WM_DESTROY, (LPARAM)m_pHostWnd->GetSafeHwnd());	// a trick using a safe and known HWND
																									// AFTER DebugWnd completes its clean up
}
...
BOOL CDebugWnd::Create(CWnd* pHostWnd)
{
	...
	m_pGraphView->Create( ((UINT) IDD_GRAPH_VIEW_DLG), this);
