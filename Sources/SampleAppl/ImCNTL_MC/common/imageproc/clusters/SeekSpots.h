...
__forceinline int _find_min_max(pTVFrame frame , 
...
__forceinline double calc_profiles( pTVFrame frame ,
  ...
  pProfile hProf , pProfile vProf )
{
  ...
  return calc_profiles( frame , hProf->m_pProfData , vProf->m_pProfData ,
    hProf->m_dMinValue , hProf->m_dMaxValue , vProf->m_dMinValue , vProf->m_dMaxValue ) ;
}
...
__forceinline bool seekSpotAlt(pTVFrame frame, DRECT& rc, 
  double * pHProf ,
  double * pVProf ,   
  DWORD White=0, //0 - Black-to-White, otherwise - white-to-black
...
__forceinline bool seekBorders(pTVFrame frame, DRECT& rc, 
   pProfile pH ,
   pProfile pV ,   
   DWORD White=0, //0 - Black-to-White, otherwise - white-to-black
   DWORD dir=3,   // 0x01 - vertical, 0x02 - horizontal 
...
__forceinline bool seekMultiBorders(pTVFrame frame, CDRectArray& Results, 
                               double * pHProf ,
                               double * pVProf ,   
...
__forceinline double _calc_integral(pTVFrame frame, CRect& rc)
{
  DWORD width = frame->lpBMIH->biWidth;
  DWORD height=frame->lpBMIH->biHeight;
  DWORD dwCompression = frame ->lpBMIH->biCompression ;
...
__forceinline double _calc_integral(
  pTVFrame frame, CPoint& Cent , int iRadius )
{
...
__forceinline double _calc_sum_over_thres(
  pTVFrame frame , CRect& rc , int iThres )
{
  DWORD width = frame->lpBMIH->biWidth;
  DWORD height=frame->lpBMIH->biHeight;
  DWORD dwCompression = frame ->lpBMIH->biCompression ;
  ...
  pProfile pHProf = NULL , pProfile pVProf = NULL )
{
  ...
  else if ( dwCompression == BI_Y16 )
  {
    ...
    for (DWORD y=0; y < height ; y++)
    {
      ...
      {
        iSumBack += iBack ;
        pVProf->m_pProfData[y] -= iBack ;  
      }
    }
    if ( pVProf  &&  pHProf )
    {
      double dAveBack = (double)iSumBack/(double)height ;
