...
__forceinline LPBITMAPINFOHEADER _cdiminish(pTVFrame frame)
{
...
__forceinline bool _diminish(pTVFrame frame)
{
    ...
    case BI_Y8:
        {
            LPBITMAPINFOHEADER lpNewBM=_cdiminish(frame);
            if (lpNewBM)
            {
                if (frame->lpData)
                    free(frame->lpData);
...
__forceinline bool _enlargeY8(pTVFrame frame)
{
    if (!frame) return false;
    if (!frame->lpBMIH) return false;
    ...
    if (frame->lpBMIH->biCompression==BI_YUV9)
    {
        ...
        src+=oldwidth*oldheight;
        int width2=width>>2;
        memset(_dst,128,width*height>>3);
    }
...
__forceinline bool _enlargeY16(pTVFrame frame)
{
    if (!frame) return false;
    if (!frame->lpBMIH) return false;
...
__forceinline bool _enlargeY(pTVFrame frame)
{
    ...
    case BI_YUV9:
    case BI_Y8:
            return _enlargeY8(frame);
    case BI_Y16:
            return _enlargeY16(frame);
    }
    return false;
}
...
__forceinline bool _resample8(pTVFrame frame, int newWidth, int newHeight)
{
    ...
    for (int y=0; y<newHeight; y++)
    {
        ...
        for (int x=0; x<newWidth; x++)
        {
            ...
            //int soff=(int)(floor(xr*x)+floor(yr*y)*oldWidth);
            int soff=x0+y0*oldWidth;
            double errX=xr*x-floor(xr*x);
            double errY=yr*y-floor(yr*y);
            //dst[x+y*newWidth]=src[x0+y0*oldWidth];
