...
__forceinline void _nearest_neighbours(LPBYTE outD,LPBYTE inD, int width, int depth)
{
  ...
  for (i=1; i<width-1; i++)
	  for (j=1; j<depth-1; j++)
  {
       outD[i+width*j]=(inD[i-1+width*j]+inD[i+1+width*j]+inD[i+width*j-width]+inD[i+width*j+width])>>2;
  }
}
...
__forceinline void _erode(int width, int depth, LPBYTE dst, LPBYTE src)
{
	...
	for (i=0; i<depth; i++)
	{
        minY=i-AREA; if (minY<0)  minY=0;
        maxY=i+AREA; if (maxY>=depth) maxY=depth-1;
		for (int j=0; j<width; j++)
		{
            minX=j-AREA; if (minX<0)  minX=0;
            maxX=j+AREA; if (maxX>=width) maxX=width-1;
            int r=0;
            Val=0;
...
__forceinline void _dilate(int width, int depth, LPBYTE dst, LPBYTE src)
{
    #define AREA 1
	if ((!src) || (!dst)) return;
	int offset=0,i;
...
__forceinline void _dilate16(int width, int depth, LPWORD dst, LPWORD src)
{
    #define AREA 1
	if ((!src) || (!dst)) return;
	int offset=0,i;
...
__forceinline bool _dilate(pTVFrame frame)
{
    switch (frame->lpBMIH->biCompression)
    {
    case BI_YUV9:
    case BI_Y8:
        {
            int imgsize=frame->lpBMIH->biWidth*frame->lpBMIH->biHeight;
            LPBYTE dst=(LPBYTE)malloc(imgsize);
            _dilate(frame->lpBMIH->biWidth,frame->lpBMIH->biHeight,dst,GetData(frame));
            memcpy(GetData(frame),dst,imgsize);
            free(dst);
            return true;
        }
    case BI_Y16:
        {
            int imgsize=frame->lpBMIH->biWidth*frame->lpBMIH->biHeight;
            LPWORD dst=(LPWORD)malloc(imgsize*sizeof(WORD));
            _dilate16(frame->lpBMIH->biWidth,frame->lpBMIH->biHeight,dst,(LPWORD)GetData(frame));
            memcpy(GetData(frame),dst,imgsize*sizeof(WORD));
            free(dst);
            return true;
        }
...
__forceinline void _erodev(int width, int depth, LPBYTE dst, LPBYTE src)
{
	...
	for (i=0; i<depth; i++)
	{
		...
		for (int j=0; j<width; j++)
		{
            ...
            for (int y=minY; y<=maxY; y++)
            {
                if (Val<src[j+y*width]) Val=src[j+y*width];
            }
            dst[offset+j]=Val; //(ValL+ValR)/2;
		}
		offset+=width;
...
__forceinline void _and_rh(int width, int depth, LPBYTE dst, LPBYTE src)
{
	...
	for (i=0; i<depth-1; i++)
	{
		for(j=0; j<width; j++)
		{
			...
			pntr=&(src[j+i*width]);
			dst[j+i*width]=(*pntr>*(pntr+width))?*(pntr+width):*pntr;
		}
	}
	memcpy(dst+i*width,dst+(i-1)*width,width);
}
