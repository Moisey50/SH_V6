...
public:
  ...
  ChainsSet m_BorderChains ;

  virtual ~CColorSpot()
  {
    for ( int i = 0 ; i < m_BorderChains.GetCount() ; i++ )
      delete m_BorderChains[i] ;
  }
  ...
  CColorSpot( CColorSpot& Orig /*, bool bDetailed = false*/ )
  {
...
public:
  ...
  int         FindSpotsByChains( pTVFrame pFrame , BOOL bDetailed = FALSE ) ;
