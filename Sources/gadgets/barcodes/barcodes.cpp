// barcodes.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE barcodesDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("barcodes.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(barcodesDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(barcodesDll);
        // Init plugin
        FXString fName=::FxGetModuleName(barcodesDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("barcodes.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(barcodesDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
     ...
     REGISTER_RUNTIME_GADGET(UPCReader, Builder);
     REGISTER_RUNTIME_GADGET(Code39Reader, Builder);
     REGISTER_RUNTIME_GADGET(Code128Reader, Builder);
     REGISTER_RUNTIME_GADGET(DataMatrixReader, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(UPCReader, Builder);
	UNREGISTER_RUNTIME_GADGET(Code39Reader, Builder);
	UNREGISTER_RUNTIME_GADGET(Code128Reader, Builder);
	UNREGISTER_RUNTIME_GADGET(DataMatrixReader, Builder);
}
...
helpitem helpitems[]={
    {"UPCReader",        IDR_HTML_10000},
    {"Code39Reader",     IDR_HTML_10001},
    {"Code128Reader",    IDR_HTML_10002},
    {"DataMatrixReader", IDR_HTML_10003}
};
