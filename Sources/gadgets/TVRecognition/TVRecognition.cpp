// TVRecognition.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE TVRecognitionDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("TVRecognition.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(TVRecognitionDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(TVRecognitionDll);
        // Init plugin
        FXString fName=::FxGetModuleName(TVRecognitionDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("TVRecognition.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(TVRecognitionDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(OCR, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(OCR, Builder);
}
