// ScanGadget.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include "scangadget.h"
#include "scan.h"
...
static AFX_EXTENSION_MODULE ScanGadgetDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ScanGadget.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(ScanGadgetDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(ScanGadgetDll);
        // Init plugin
        FXString fName=::FxGetModuleName(ScanGadgetDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ScanGadget.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(ScanGadgetDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(Scan, Builder);
}

_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(Scan, Builder);
}
