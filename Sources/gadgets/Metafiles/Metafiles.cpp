// Metafiles.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE MetafilesDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Metafiles.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(MetafilesDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(MetafilesDll);
        // Init plugin
        FXString fName=::FxGetModuleName(MetafilesDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Metafiles.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(MetafilesDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	IGraphBuilderEx* Builder = (IGraphBuilderEx*)pGraphBuilder;
     REGISTER_RUNTIME_GADGET(WMF, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(WMF, Builder);
}
