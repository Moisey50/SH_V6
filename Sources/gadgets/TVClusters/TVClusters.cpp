// TVClusters.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE TVClustersDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("TVClusters.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(TVClustersDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(TVClustersDll);
        // Init plugin
        FXString fName=::FxGetModuleName(TVClustersDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("TVClusters.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(TVClustersDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(Lines, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
    UNREGISTER_RUNTIME_GADGET(Lines, Builder);
}
