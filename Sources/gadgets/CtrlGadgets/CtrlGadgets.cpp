// CtrlGadgets.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE CtrlGadgetsDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("CtrlGadgets.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(CtrlGadgetsDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(CtrlGadgetsDll);
        // Init plugin
        FXString fName=::FxGetModuleName(CtrlGadgetsDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("CtrlGadgets.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(CtrlGadgetsDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
     ...
     REGISTER_RUNTIME_GADGET(Knob, Builder);
     REGISTER_RUNTIME_GADGET(CheckBoxCtrl, Builder);
     REGISTER_RUNTIME_GADGET(TextCaptureCtrl, Builder);
     REGISTER_RUNTIME_GADGET(ScrollCtrl, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
     ...
     UNREGISTER_RUNTIME_GADGET(Knob, Builder);
     UNREGISTER_RUNTIME_GADGET(CheckBoxCtrl, Builder);
     UNREGISTER_RUNTIME_GADGET(TextCaptureCtrl, Builder);
     UNREGISTER_RUNTIME_GADGET(ScrollCtrl, Builder);
}
...
{	
    {"Knob",            IDR_HTML_10000},
    {"CheckBoxCtrl",    IDR_HTML_10001},
    {"TextCaptureCtrl", IDR_HTML_10002},
    {"ScrollCtrl",      IDR_HTML_10003}
};
