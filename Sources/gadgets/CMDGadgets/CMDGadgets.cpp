// CMDGadgets.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE CMDGadgetsDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("CMDGadgets.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(CMDGadgetsDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(CMDGadgetsDll);
        // Init plugin
        FXString fName=::FxGetModuleName(CMDGadgetsDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("CMDGadgets.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(CMDGadgetsDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
     ...
     REGISTER_RUNTIME_GADGET( Command , Builder );
     REGISTER_RUNTIME_GADGET( PostMsg , Builder );
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
     ...
     UNREGISTER_RUNTIME_GADGET(Command, Builder);
     UNREGISTER_RUNTIME_GADGET( PostMsg , Builder );
}
