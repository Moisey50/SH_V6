// Network.cpp : Defines the initialization routines for the DLL.
//
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Network.DLL Initializing!\n");
		...
		pThisDll = new CDynLinkLibrary(NetworkDll);
        // Init plugin
        FXString fName=::FxGetModuleName(NetworkDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Network.DLL Terminating!\n");
...
// Add filter collection to graph builder:
    ...
    REGISTER_RUNTIME_GADGET(NWSocket, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
    ...
    UNREGISTER_RUNTIME_GADGET(NWSocket, Builder);
}
