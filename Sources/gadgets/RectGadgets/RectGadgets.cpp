// RectGadgets.cpp : Defines the initialization routines for the DLL.
//

#include "stdafx.h"
#include "rectgadgets.h"
...
static AFX_EXTENSION_MODULE RectGadgetsDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("RectGadgets.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(RectGadgetsDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(RectGadgetsDll);
        // Init plugin
        FXString fName=::FxGetModuleName(RectGadgetsDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("RectGadgets.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(RectGadgetsDll);
	}
	return 1;   // ok
}
...
// Add filter collection to graph builder:
	...
	REGISTER_RUNTIME_GADGET(InflateRect, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(InflateRect, Builder);
}

_declspec(dllexport) const char* __stdcall GetPluginName()
{
