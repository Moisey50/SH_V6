// ptgrey2_0.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE ptgrey2_0Dll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ptgrey2_0.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(ptgrey2_0Dll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(ptgrey2_0Dll);
        // Init plugin
        FXString fName=::FxGetModuleName(ptgrey2_0Dll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ptgrey2_0.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(ptgrey2_0Dll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(PtGrey2_0, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
    ...
    UNREGISTER_RUNTIME_GADGET(PtGrey2_0, Builder);
}
