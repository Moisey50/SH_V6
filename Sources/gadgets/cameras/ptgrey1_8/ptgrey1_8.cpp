// ptgrey1_8.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE ptgrey1_8Dll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ptgrey1_8.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(ptgrey1_8Dll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(ptgrey1_8Dll);
        // Init plugin
        FXString fName=::FxGetModuleName(ptgrey1_8Dll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ptgrey1_8.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(ptgrey1_8Dll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(PtGrey1_8, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(PtGrey1_8, Builder);
}
