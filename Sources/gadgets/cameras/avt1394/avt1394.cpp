// avt3_11.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE AVTFDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("AVTF.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(AVTFDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(AVTFDll);
        // Init plugin
        FXString fName=::FxGetModuleName(AVTFDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("AVTF.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(AVTFDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
    ...
    REGISTER_RUNTIME_GADGET(AVTF, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
    ...
    UNREGISTER_RUNTIME_GADGET(AVTF, Builder);
}
