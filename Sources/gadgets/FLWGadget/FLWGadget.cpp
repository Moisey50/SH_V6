// FLWGadget.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE FLWGadgetDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("FLWGadget.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(FLWGadgetDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(FLWGadgetDll);
        // Init plugin
        FXString fName=::FxGetModuleName(FLWGadgetDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("FLWGadget.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(FLWGadgetDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(FLWCapture, Builder);
	REGISTER_RUNTIME_GADGET(FLWRender, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(FLWCapture, Builder);
	UNREGISTER_RUNTIME_GADGET(FLWRender, Builder);
}
...
helpitem helpitems[]=
{
	{"FLWRender",   IDR_HTML_10000},
	{"FLWCapture",  IDR_HTML_10001}
};
