...
// construction and destruction
    ...
    { 
        m_phlines=(_hline*)malloc(sizeof(_hline)*(LINE_CLUSTERS_GROWTH)); 
        if (m_phlines) 
          nmb_allocated=(LINE_CLUSTERS_GROWTH); 
        nmb_stored=0; 
    };
...
// methods
    ...
    void add(int xl,int xr,int y)
    {
        ...
        {
            m_phlines=(_hline*)realloc(m_phlines,
              sizeof(_hline)*(nmb_allocated+LINE_CLUSTERS_GROWTH)); 
...
bool CClusters::ParseFrame(const pTVFrame frame)
{
    Reset();
...
bool CClusters::SeekClusters()
{
    ...
    while(scanner<end_of_data)
    {
        ...
        {
            int offset=(int)(scanner-m_WrkData);
