...
#endif
{
    hSrc=FindResource(hModule,lpName,lpType);
    int size=0;
    if (!hSrc)
    {
        RecognitionStatus="Can't get lib handle from resource";
        TRACE("!!! Error! Can't find resource '%s'-'%s'\n",lpName,lpType);
        return 0;
    }

    gocrlib=LoadResource(hModule,hSrc);
    if (!gocrlib)
    {
    ...
    ocrlibr =(LPBYTE)LockResource(gocrlib);
    if (!ocrlibr)
    {
        DeleteObject(gocrlib);
        RecognitionStatus="Can't Allocate memory for ocr lib";
...
/*		if (fread(ocrlib,1,lh.libsize,libFilePtr)!=lh.libsize)
    ...
    RecognitionStatus="Library successfully loaded";
	}
	else 
...
{
  if (!bmax) 
    bmax=_ocr_getlib(hModule, lpName, lpType);
    return(bmax!=0);
}
...
void OCR_Save()
{
	if ((!Ask4Save) || (FxMessageBox(PROMPT_SAVEOCRLIB,MB_YESNO)==IDYES))
	{
		FILE  *libFilePtr;
