...
__forceinline pTVFrame _estimateAOI(const pTVFrame frame, RECT* rc)
{
    CRect roi=*rc;
    ...
    pTVFrame retV=makecopyTVFrame(frame);
    _diminish(retV);
...
//		idata* re=m_pDirections;
		...
		while(h<eod)
		{
			...
			if (*h==128) // almost horizontal lines
			{
				...
				while (*hs==128)
				{
					...
					if (max) 
					{
						_soi.add((int)(max-m_pMagnitude),maxV);
					}
					hs++; vs++; 
	...
	ts=GetHRTickCount()-ts;
	TRACE("+++ Making image map takes %f ms\n",ts);
	return;
}
...
__forceinline void _calc_res(
  DPOINT dA, DPOINT dB, DPOINT a, DPOINT b, DPOINT dR)
{
    double d[4];

    d[0] = abs(CDP2Cmplx(dA) - CDP2Cmplx(a)); 
    d[1] = abs(CDP2Cmplx(dA) - CDP2Cmplx(b));
    d[2] = abs(CDP2Cmplx(dB) - CDP2Cmplx(a));
    d[3] = abs(CDP2Cmplx(dB) - CDP2Cmplx(b));
