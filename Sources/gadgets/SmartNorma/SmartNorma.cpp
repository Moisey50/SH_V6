// SmartNorma.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE SmartNormaDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("SmartNorma.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(SmartNormaDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(SmartNormaDll);
        // Init plugin
        FXString fName=::FxGetModuleName(SmartNormaDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("SmartNorma.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(SmartNormaDll);
	}
	return 1;   // ok
}
...
// Add filter collection to graph builder:
    ...
    REGISTER_RUNTIME_GADGET(AnchorNormalization, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	UNREGISTER_RUNTIME_GADGET(AnchorNormalization, Builder);
}
...
helpitem helpitems[]=
{
	{"AnchorNormalization",   IDR_HTML_10000}
};
