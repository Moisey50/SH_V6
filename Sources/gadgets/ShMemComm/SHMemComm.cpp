...
static AFX_EXTENSION_MODULE SHMemGadgetsDll = { NULL , NULL };
...
DllMain( HINSTANCE hInstance , DWORD dwReason , LPVOID lpReserved )
{
  ...
  {
    TRACE0( "SHMemGadgets.DLL Initializing!\n" );

    // Extension DLL one-time initialization
    if ( !AfxInitExtensionModule( SHMemGadgetsDll , hInstance ) )
      return 0;
    ...
    pThisDll = new CDynLinkLibrary( SHMemGadgetsDll );
        // Init plugin
    FXString fName = ::FxGetModuleName( SHMemGadgetsDll.hModule );
    ASSERT( fName.GetLength() < APP_NAME_MAXLENGTH );
    strcpy_s( TVDB400_PLUGIN_NAME , fName );
  }
  else if ( dwReason == DLL_PROCESS_DETACH )
  {
    TRACE0( "SHMemGadgets.DLL Terminating!\n" );
    // Terminate the library before destructors are called
    AfxTermExtensionModule( SHMemGadgetsDll );
  }
  return 1;   // ok
}
...
// Add filter collection to graph builder:
  ...
  REGISTER_RUNTIME_GADGET( ShMemComm , Builder );
}
...
_declspec( dllexport ) void __stdcall PluginExit( LPVOID pGraphBuilder )
{
  ...
  UNREGISTER_RUNTIME_GADGET( ShMemComm , Builder );
}
