// TVAvi.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE TVAviDll = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("TVAvi.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(TVAviDll, hInstance))
			return 0;
		...
		pThisDll = new CDynLinkLibrary(TVAviDll);
        // Init plugin
        FXString fName=::FxGetModuleName(TVAviDll.hModule);
        ASSERT(fName.GetLength()<APP_NAME_MAXLENGTH);
        strcpy(TVDB400_PLUGIN_NAME, fName);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("TVAvi.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(TVAviDll);
	}
	return 1;   // ok
}
...
_declspec(dllexport) void __stdcall PluginEntry(LPVOID pGraphBuilder)
{
	...
	REGISTER_RUNTIME_GADGET(AviCapture, Builder);
	REGISTER_RUNTIME_GADGET(AviRender, Builder);
}
...
_declspec(dllexport) void __stdcall PluginExit(LPVOID pGraphBuilder)
{
	...
	IGraphbuilder* Builder = (IGraphbuilder*)pGraphBuilder;
	UNREGISTER_RUNTIME_GADGET(AviCapture, Builder);
    UNREGISTER_RUNTIME_GADGET(AviRender, Builder);
}
...
helpitem helpitems[]=
{
	{"AviRender",   IDR_HTML_10000},
	{"AviCapture",  IDR_HTML_10001}	
};
