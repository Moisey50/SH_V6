...
//           } while ( *(AreaName++) );
  ...
  LPCTSTR GetErrorString( int iErrCode )
  {
    ...
    if ( iErrCode >= 0 )
      sprintf_s( m_OutMsg , "Transfered %d bytes" , iErrCode ) ;
    else
      sprintf_s( m_OutMsg , "Error code %d " , iErrCode ) ;
    return m_OutMsg ;
  }
...
protected:
  ...
  int       m_iNotNewArea ;
  int       m_iMsgCnt ;
  DWORD     m_dwTimeoutForTransfer ; // 1 second
