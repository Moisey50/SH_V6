...
{
  if ( _tcscpy_s( pBuf , iBufLenElements , ("Software\\") ) )
    return NULL ;
  if ( _tcscat_s( pBuf , iBufLenElements , m_KeyPrefix ) )
    return NULL;
  if ( _tcscat_s( pBuf , iBufLenElements , ("\\") ) )
    return NULL;
    ...
    LPCTSTR Key, LPCTSTR ValueName, LPCTSTR  Default) 
{
  ...
  if ( m_KeyCreated )
  {
    int iRes = RegSetValue(  
      FileCompanyKey , ValueName , dwType , Default , 
      ...
      LPCTSTR Key, LPCTSTR ValueName, double dDefault) 
{
  TCHAR szDef[ 100 ] ;
  double dResult = 1e30 ;
  _stprintf( szDef , ("%g") , dDefault ) ;

  _stscanf( GetRegiString( Key , ValueName , szDef ) , ("%lg") , &dResult ) ;
   ...
   LPCTSTR Key, LPCTSTR ValueName, __int64 i64Default) 
{
  ...
  _stprintf( szDef , ("%I64d") , i64Default ) ;

  _stscanf( GetRegiString( Key , ValueName , szDef ) , ("%I64d") , &i64Result ) ;
...
{
  TCHAR * p = GetRegiString( Key , ValueName , ("") ) ;

  if ( !p  ||  !(*p) )
    return 0 ;

  TCHAR * tok = _tcstok( ( TCHAR * )p , (" ,") );
  ...
  while ( tok )
  {
    ...
    int iRes ;
    if ( (tok[0] == ('0'))  &&  (_totlower(tok[1]) == ('x')) )
      iRes = _stscanf( tok + 2 , ("%x") , &dwNewNumber ) ;
    else 
      iRes = _stscanf( tok , ("%d") , &dwNewNumber ) ;
