...
::YouTube:
::Description: Add/change 'SH_SOURCES" System 
::             enviornment variable. Elevates 
::             to Admin if nesseceary. 
::
::Revisions History
::
::Rev.001 Date: 10/10/2018 Cause: Fixed for folder
::                                with spaces.
:: 
::**********************************************
@Echo Off
TITLE Add/change 'SH_SOURCES" System Enviornment Variable
...
Set "_sysVarName=SH_SOURCES"
Set "_sysVarValue=%cd%"

Echo SETX %_sysVarName% %_sysVarValue% /m called.
SETX %_sysVarName% "%_sysVarValue%" /m >%temp%\~SetXRes.txt
...
if defined _setVarRes (
  ...
  Echo %_sysVarName%=%_sysVarValue%
  Echo %_setVarRes%
) ELSE (
  Echo "SETX %_sysVarName% %_sysVarValueNew% /m  -- Action failed!"
)
